...
class sql
{
    ...
    private function get()
    {
        if(!$this->pdo)
        {
            $this->pdo = new PDO("{$this->driver}:host={$this->server};dbname={$this->database};charset={$this->charset}", $this->username, $this->password, [PDO::ATTR_PERSISTENT => false]);
        }
        return $this->pdo;
    }
    ...
    private function error($message)
    {
        ...
        $this->pdo = null;
        backend_error('bad_query', $message);
    }
